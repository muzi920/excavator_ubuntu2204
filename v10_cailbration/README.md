...
- **使用**：阅读该文档以了解当前模型的参数设定。**注意顶部的 TODO 项**，在进行下一次实地测试前，需要按照 TODO 的提示测量小臂和铲斗的初始物理姿态，以完成最终的标定。
...
- **使用方法**：
  ...
  2. 运行（支持输入相对于 `src/shandong` 根目录的路径）：
     ...
     ```
  3. 脚本会在当前目录生成与 JSON 文件同名的 GIF 动画（例如 `script.gif`），你可以在图片查看器中打开以预览动作是否连贯、是否符合预期。