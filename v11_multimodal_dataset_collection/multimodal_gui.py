...
class V11MultimodalGUI:
    ...
    def _start_camera_threads(self):
        ...
        self.camera_threads.extend([t_hik, t_net1, t_net2])
    ...
    def _imu_listener_loop(self):
        ...
        while self.imu_running:
            ...
            elif (data[0] == 0x00 or data[0] == 0x01) and len(data) >= 36:
                header = struct.unpack_from('<B H H H H B B B 12s I Q', data, 0)
                dot_num = header[3]
                data_type = header[6]
                
                if data_type == LIDARPOINTCLOUD and self.recorder.is_recording:
                    # 将本次报文解析成 numpy 数组 (只取 x, y, z)
                    points = []
                    offset = 36
                    for i in range(dot_num):
                        if offset + 8 > len(data):
                            break
                        point_data = struct.unpack_from('<I I B B', data, offset)
                        word1, word2 = point_data[0], point_data[1]
                    ...
                    if points:
                        # 点云报文非常高频，我们抽样保存，比如 1/10 避免把磁盘写爆
                        pc_count += 1
                        if pc_count % 10 == 0:
                            ts = time.time()
                            # 开新线程异步落盘
                            threading.Thread(target=self.recorder.save_pointcloud, args=(ts, np.array(points, dtype=np.float32)), daemon=True).start()
    ...
    def _open_live_3d_window(self):
        if self.live_3d_window is not None and tk.toplevel.winfo_exists(self.live_3d_window):
