...
class DemoDataset(DatasetTemplate):
    ...
    def __getitem__(self, index):
        ...
        # 如果提供了 labels 目录，加载该帧对应的真实标签
        if self.labels_dir is not None:
            sample_name = Path(self.sample_file_list[index]).stem
            ...
            if label_file.exists():
                ...
                with open(label_file, 'r') as f:
                    for line in f.readlines():
                        ...
                        if len(parts) >= 8:
                            # CustomDataset 格式: x y z l w h angle class
                            gt_boxes.append([float(x) for x in parts[:7]])
...
def parse_config():
    ...
    parser.add_argument('--labels_dir', type=str, default=None,
                        help='specify the labels directory for ground-truth visualization')
...
def main():
    ...
    with torch.no_grad():
        for idx, data_dict in enumerate(demo_dataset):
            ...
            # collate_batch 会在 gt_boxes 上加 batch 维并补零，需要去掉
            gt_boxes = data_dict.get('gt_boxes', None)
            if gt_boxes is not None:
                gt_boxes = gt_boxes[0]                         # (batch, max_gt, 7) → (max_gt, 7)
                gt_boxes = gt_boxes[gt_boxes.sum(axis=1) != 0]  # 过滤填充的零框
                if len(gt_boxes) == 0:
                    gt_boxes = None
