...
class NetCamsPublisher(Node):
    def __init__(self):
        super().__init__('net_cams_publisher')
        ...
        net2_url = self.get_parameter('net2_rtsp_url').value
        ...
        self.pub_net2 = self.create_publisher(Image, 'network_cam2/image_raw', 10)
        self.pub_hik = self.create_publisher(Image, 'hikvision_cam/image_raw', 10)
        
        # ================= 3. 依次初始化摄像头 =================
        self.get_logger().info('正在初始化网络摄像头...')
        
        # 3.1 普通网络摄像头1 (UDP 传输，防卡死)
        ...
        if self.cap_net.isOpened():
            self.get_logger().info(f'[成功] 网络摄像头1已连接: {net_url}')
        else:
            self.get_logger().error(f'[失败] 网络摄像头1连接失败: {net_url}')
            
        # 3.2 普通网络摄像头2 (UDP 传输，防卡死)
        self.cap_net2 = cv2.VideoCapture(net2_url, cv2.CAP_FFMPEG)
        if self.cap_net2.isOpened():
            self.get_logger().info(f'[成功] 网络摄像头2已连接: {net2_url}')
        else:
            self.get_logger().error(f'[失败] 网络摄像头2连接失败: {net2_url}')
        ...
        if self.cap_net.isOpened():
            t1 = threading.Thread(target=self.capture_loop, args=(self.cap_net, self.pub_net, "network_cam_frame", True))
            self.threads.append(t1)
            t1.start()
            
        if self.cap_net2.isOpened():
            t2 = threading.Thread(target=self.capture_loop, args=(self.cap_net2, self.pub_net2, "network_cam2_frame", True))
    ...
    def destroy_node(self):
        ...
        if hasattr(self, 'cap_net2') and self.cap_net2.isOpened(): self.cap_net2.release()
        if hasattr(self, 'cap_hik') and self.cap_hik.isOpened(): self.cap_hik.release()
        
        self.get_logger().info('所有网络摄像头资源已安全释放')
        super().destroy_node()

def main(args=None):
    rclpy.init(args=args)
    node = NetCamsPublisher()
