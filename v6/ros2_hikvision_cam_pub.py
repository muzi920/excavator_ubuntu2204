...
class HikvisionCamPublisher(Node):
    def __init__(self):
        super().__init__('hikvision_cam_publisher')
        ...
        # 增加 stimeout 参数防止断网卡死
        os.environ["OPENCV_FFMPEG_CAPTURE_OPTIONS"] = "rtsp_transport;tcp|stimeout;3000000"
        
        # 创建发布者
        self.publisher_ = self.create_publisher(Image, 'hikvision_cam/image_raw', 10)
        
        # 初始化 cv_bridge
        self.br = CvBridge()
        
        # 打开海康摄像头
        self.get_logger().info(f'尝试连接海康摄像头 (RTSP over TCP): {rtsp_url}')
        self.cap = cv2.VideoCapture(rtsp_url, cv2.CAP_FFMPEG)
        
        if not self.cap.isOpened():
            self.get_logger().error('无法连接到海康 RTSP 流。请检查网络和地址。')
            return
    ...
    def timer_callback(self):
        ...
        if ret:
            ...
            msg.header.frame_id = "hikvision_cam_frame"
